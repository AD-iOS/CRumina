...
namespace vm_constants {
    ...
    const char* ERR_INVALID_CONST_INDEX = "Invalid constant pool index";
    const char* ERR_ARRAY_INDEX_MUST_BE_INT = "Array index must be an integer";
    const char* ERR_STRING_INDEX_MUST_BE_INT = "String index must be an integer";
    const char* ERR_CANNOT_INDEX_TYPE = "Cannot index type";
    const char* ERR_CANNOT_CALL_TYPE = "Cannot call type";
    const char* ERR_BREAK_OUTSIDE_LOOP = "Break outside of loop";
    const char* ERR_CONTINUE_OUTSIDE_LOOP = "Continue outside of loop";
    const char* ERR_LAMBDA_ID_NOT_FOUND = "Lambda ID not found";
} /* namespace vm_constants */
...
size_t ByteCode::addConstant(const Value& value) {
    // 创建缓存键
    ConstantKey key(nullptr);
    
    switch (value.getType()) {
        case Value::Type::Int:
            key = ConstantKey(value.getInt());
            break;
        case Value::Type::Float:
            key = ConstantKey(value.getFloat());
            break;
        case Value::Type::Bool:
            key = ConstantKey(value.getBool());
            break;
        case Value::Type::String:
            key = ConstantKey(value.getString());
            break;
        case Value::Type::Null:
            key = ConstantKey(nullptr);
            break;
        default:
            // 对复杂类型不使用缓存
...
void VM::executeInstructionAt(size_t ip) {
    ...
    switch (op.type) {
        ...
        case OpCodeType::Member: {
            size_t cache_addr = ip - 1; // 当前指令地址
            if (stack_.empty()) throw std::runtime_error(vm_constants::ERR_STACK_UNDERFLOW);
            ...
            if (object.getType() == Value::Type::Struct ||
                ...
                if (it != struct_map->end()) {
                    ...
                    if (cache_it != member_cache_.end()) {
                        ...
                        member_cache_.emplace(cache_addr, InlineCache(member, 1, 0));
                    }
                    ...
                    if (cache_it != member_cache_.end()) {
                        ...
                        member_cache_.emplace(cache_addr, InlineCache(member, 0, 1));
                    }
                    throw std::runtime_error(object.typeName() + 
                        " does not have member '" + member + "'");
                }
            } else if (object.getType() == Value::Type::Array) {
                // 数组方法处理
                if (member == "foreach" || member == "map" || member == "filter" || 
            member == "reduce" || member == "fold" || member == "push" || member == "pop") {
            
                    auto git = globals_->find(member);
                    if (git != globals_->end()) {
                        stack_.push_back(git->second);
                    } else {
                        throw std::runtime_error("Undefined array method '" + member + "'");
                    }
                } else {
                    auto cache_it = member_cache_.find(cache_addr);
                    if (cache_it != member_cache_.end()) {
                    ...
                    throw std::runtime_error("array does not have member '" + member + "'");
                }
            } else {
                auto cache_it = member_cache_.find(cache_addr);
                if (cache_it != member_cache_.end()) {
            ...
            if (func.getType() == Value::Type::NativeFunction) {
                ...
                if (recursion_depth_ >= MAX_RECURSION_DEPTH) {
                    throw std::runtime_error("Maximum recursion depth (" + 
                          std::to_string(MAX_RECURSION_DEPTH) + ") exceeded");
                }
                // 从 lambda body 中提取 lambda_id
                std::string lambda_id;
                if (lambda.body) {
                    if (auto include_stmt = dynamic_cast<IncludeStmt*>(lambda.body.get())) {
                ...
                if (lambda_id.empty()) {
                    throw std::runtime_error(vm_constants::ERR_LAMBDA_ID_NOT_FOUND);
                }
                auto it = functions_.find(lambda_id);
                if (it == functions_.end()) {
                    throw std::runtime_error("Lambda '" + lambda_id + "' not found");
                }
                const FuncDefInfo& info = it->second;
                // 创建调用帧
                CallFrame frame;
        ...
        */
        ...
        case OpCodeType::MakeLambda: {
            ...
            // 使用 IncludeStmt 作为标记存储 lambda_id
