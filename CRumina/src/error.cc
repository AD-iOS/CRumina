...
RuminaError RuminaError::syntaxError(const std::string& msg) {
    return RuminaError(RuminaErrorType::SyntaxError, msg);
}

RuminaError RuminaError::arityError(const std::string& func_name, size_t expected, size_t got) {
    std::string msg = "Function '" + func_name + "' expects " + 
                      std::to_string(expected) + " arguments, got " + 
                      std::to_string(got);
    return RuminaError(RuminaErrorType::ArityError, msg);
}
...
std::string RuminaError::formatError() const {
    ...
    oss << error_type_to_string(type_) << ": " << message_ << "\n";
