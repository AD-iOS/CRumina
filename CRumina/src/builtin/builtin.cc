...
// 添加 using 声明
using namespace rumina::builtin::set;

namespace set = rumina::builtin::set;

namespace rumina {
namespace builtin {

void register_builtins(std::unordered_map<std::string, Value>& globals) {
    if (globals.find("size") == globals.end()) {
    ...
    if (globals.find("assert") == globals.end()) {
        globals["assert"] = Value::makeNativeFunction("assert", utils::assert_fn);
    }

    globals["set::get"] = Value::makeNativeFunction("set::get", set::set_get);
    globals["set::mainValue"] = Value::makeNativeFunction("set::mainValue", set::set_main_value);
    globals["set::getReal"] = Value::makeNativeFunction("set::getReal", set::set_get_real);
    globals["set::add"] = Value::makeNativeFunction("set::add", set::set_to_add);
    globals["set::sub"] = Value::makeNativeFunction("set::sub", set::set_to_sub);
    globals["set::mul"] = Value::makeNativeFunction("set::mul", set::set_to_multiply);
    globals["set::div"] = Value::makeNativeFunction("set::div", set::set_to_divide);
    globals["set::pow"] = Value::makeNativeFunction("set::pow", set::set_to_pow);
    globals["set::sqrt"] = Value::makeNativeFunction("set::sqrt", set::set_to_sqrt);
    globals["set::sin"] = Value::makeNativeFunction("set::sin", set::set_to_sin);
    globals["set::cos"] = Value::makeNativeFunction("set::cos", set::set_to_cos);
    globals["set::tan"] = Value::makeNativeFunction("set::tan", set::set_to_tangent);
    ...
    // LSR-006: Set 函数
    globals["Set"] = Value::makeNativeFunction("Set", set::set_constructor);
    globals["set_get"] = Value::makeNativeFunction("set_get", set::set_get);
    globals["set_main_value"] = Value::makeNativeFunction("set_main_value", set::set_main_value);
    globals["set_get_real"] = Value::makeNativeFunction("set_get_real", set::set_get_real);
    globals["set_to_add"] = Value::makeNativeFunction("set_to_add", set::set_to_add);
    globals["set_to_sub"] = Value::makeNativeFunction("set_to_sub", set::set_to_sub);
    globals["set_to_multiply"] = Value::makeNativeFunction("set_to_multiply", set::set_to_multiply);
    globals["set_to_divide"] = Value::makeNativeFunction("set_to_divide", set::set_to_divide);
    globals["set_to_pow"] = Value::makeNativeFunction("set_to_pow", set::set_to_pow);
    globals["set_to_sqrt"] = Value::makeNativeFunction("set_to_sqrt", set::set_to_sqrt);
    globals["set_to_sin"] = Value::makeNativeFunction("set_to_sin", set::set_to_sin);
    globals["set_to_cos"] = Value::makeNativeFunction("set_to_cos", set::set_to_cos);
    globals["set_to_tangent"] = Value::makeNativeFunction("set_to_tangent", set::set_to_tangent);

    // Set.isReturnSet 全局标志
    auto set_module = std::make_shared<std::unordered_map<std::string, Value>>();
    (*set_module)["isReturnSet"] = Value(false);
    (*set_module)["constructor"] = Value::makeNativeFunction("Set", set::set_constructor);
    globals["Set"] = Value::makeModule(set_module);
    // fold
    globals["fold"] = Value::makeNativeFunction("fold", array::fold);
}
...
// convert_to_declared_type 的完整实现
...
Value convert_to_rational(const std::vector<Value>& args) {
    ...
    switch (val.getType()) {
        ...
        case Value::Type::Float: {
            ...
            if (!std::isfinite(f)) {
                throw std::runtime_error("Cannot convert infinite or NaN to rational");
            }
            ...
            for (int i = 0; i < MAX_ITERATIONS; ++i) {
                ...
                double approx = static_cast<double>(hp) / kp;
            ...
            for (int precision = 1; precision <= 12; ++precision) {
                ...
                if (std::abs(scaled - rounded) < EPSILON) {
                    ...
                    long long gcd_val = std::gcd(static_cast<long long>(std::abs(num)), 
        ...
        case Value::Type::String: {
            ...
            if (slash_pos != std::string::npos) {
                ...
                try {
                    ...
                    long den = std::stol(den_str);
                    if (den != 0) {
                        return Value(BigRational(num, den));
                    }
                } catch (...) {}
            }
            throw std::runtime_error("Cannot convert string '" + s + "' to rational");
        }
        default:
    ...
    switch (val.getType()) {
        ...
        case Value::Type::String: {
            ...
            if (i_pos != std::string::npos) {
                if (plus_pos != std::string::npos && plus_pos < i_pos) {
                    // a + bi 形式
                    std::string re_str = s.substr(0, plus_pos);
                    std::string im_str = s.substr(plus_pos + 1, i_pos - plus_pos - 1);
                    
                    double re = std::stod(re_str);
                    double im = std::stod(im_str);
                    
                    return Value(
                        std::make_shared<Value>(Value(re)),
                        std::make_shared<Value>(Value(im))
                    );
                } else {
                    // bi 或 i 形式
                    double im = 1.0;
                    if (i_pos > 0) {
                        std::string im_str = s.substr(0, i_pos);
                        im = std::stod(im_str);
                    }
                    return Value(
                        std::make_shared<Value>(Value(0.0)),
            ...
            throw std::runtime_error("Cannot convert string '" + s + "' to complex");
        }
        default:
