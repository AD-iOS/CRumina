...
Result<Value> value_binary_op(const Value& left, BinOp op, const Value& right) {
    try {
        ...
        if (left.getType() == Value::Type::Complex && right.getType() == Value::Type::Complex) {
            auto [re1, im1] = left.getComplex();
            auto [re2, im2] = right.getComplex();
            switch (op) {
                case BinOp::Add: {
                    auto re = value_binary_op(*re1, BinOp::Add, *re2);
                    if (re.is_error()) return Err<Value>(re.error());
                    auto im = value_binary_op(*im1, BinOp::Add, *im2);
                    if (im.is_error()) return Err<Value>(im.error());
                    return Ok(Value(
                        std::make_shared<Value>(re.value()),
                        std::make_shared<Value>(im.value())
                    ));
                }
                case BinOp::Sub: {
                    auto re = value_binary_op(*re1, BinOp::Sub, *re2);
                    if (re.is_error()) return Err<Value>(re.error());
                    auto im = value_binary_op(*im1, BinOp::Sub, *im2);
                    if (im.is_error()) return Err<Value>(im.error());
                    return Ok(Value(
                        std::make_shared<Value>(re.value()),
                        std::make_shared<Value>(im.value())
                ...
                case BinOp::Mul: {
                    ...
                    auto ac = value_binary_op(*re1, BinOp::Mul, *re2);
                    if (ac.is_error()) return Err<Value>(ac.error());
                    auto bd = value_binary_op(*im1, BinOp::Mul, *im2);
                    if (bd.is_error()) return Err<Value>(bd.error());
                    auto ad = value_binary_op(*re1, BinOp::Mul, *im2);
                    if (ad.is_error()) return Err<Value>(ad.error());
                    auto bc = value_binary_op(*im1, BinOp::Mul, *re2);
                    if (bc.is_error()) return Err<Value>(bc.error());
                    auto re = value_binary_op(ac.value(), BinOp::Sub, bd.value());
                    if (re.is_error()) return Err<Value>(re.error());
                    auto im = value_binary_op(ad.value(), BinOp::Add, bc.value());
                    if (im.is_error()) return Err<Value>(im.error());      
                    return Ok(Value(
                        std::make_shared<Value>(re.value()),
                        std::make_shared<Value>(im.value())
                ...
                case BinOp::Div: {
                    ...
                    auto c_sq = value_binary_op(*re2, BinOp::Mul, *re2);
                    if (c_sq.is_error()) return Err<Value>(c_sq.error());
                    auto d_sq = value_binary_op(*im2, BinOp::Mul, *im2);
                    if (d_sq.is_error()) return Err<Value>(d_sq.error());
                    auto denom = value_binary_op(c_sq.value(), BinOp::Add, d_sq.value());
                    if (denom.is_error()) return Err<Value>(denom.error());
                    auto ac = value_binary_op(*re1, BinOp::Mul, *re2);
                    if (ac.is_error()) return Err<Value>(ac.error());
                    auto bd = value_binary_op(*im1, BinOp::Mul, *im2);
                    if (bd.is_error()) return Err<Value>(bd.error());
                    ...
                    auto ad = value_binary_op(*re1, BinOp::Mul, *im2);
                    if (ad.is_error()) return Err<Value>(ad.error());
                    auto re_num = value_binary_op(ac.value(), BinOp::Add, bd.value());
                    if (re_num.is_error()) return Err<Value>(re_num.error());
                    auto im_num = value_binary_op(bc.value(), BinOp::Sub, ad.value());
                    if (im_num.is_error()) return Err<Value>(im_num.error());    
                    auto re = value_binary_op(re_num.value(), BinOp::Div, denom.value());
                    if (re.is_error()) return Err<Value>(re.error());
                    auto im = value_binary_op(im_num.value(), BinOp::Div, denom.value());
                    if (im.is_error()) return Err<Value>(im.error());
                    return Ok(Value(
                        std::make_shared<Value>(re.value()),
                        std::make_shared<Value>(im.value())
                ...
                case BinOp::Equal: {
                    auto re_eq = value_binary_op(*re1, BinOp::Equal, *re2);
                    if (re_eq.is_error()) return Err<Value>(re_eq.error());
                    auto im_eq = value_binary_op(*im1, BinOp::Equal, *im2);
                    if (im_eq.is_error()) return Err<Value>(im_eq.error());
                    if (re_eq.value().getType() != Value::Type::Bool ||
                        im_eq.value().getType() != Value::Type::Bool) {
                        return Err<Value>("Comparison failed");
                    }
                    return Ok(Value(re_eq.value().getBool() && im_eq.value().getBool()));
                }
                default:
                    return Err<Value>("Unsupported operation for complex numbers");
            }
        }
        // 整数快速路径
...
/*
bool should_use_parallel(uint32_t exponent) {
    return exponent >= PARALLEL_POW_THRESHOLD;
}

size_t calculate_parallel_chunks(uint32_t exponent) {
    if (exponent >= 100000) return 8;
    if (exponent >= 50000) return 6;
    if (exponent >= 20000) return 4;
    return 2;
}
*/
...
BigInt bigint_pow_parallel(const BigInt& base, uint32_t exponent) {
    ...
    // 计算并行分块
    size_t num_chunks = calculate_parallel_chunks(exponent);
    ...
    // 处理余数
    if (remainder > 0) {
        BigInt remainder_result = 1;
        BigInt b = base;
        uint32_t exp = remainder;
        while (exp > 0) {
            if (exp & 1) remainder_result *= b;
            b *= b;
            exp >>= 1;
        }
