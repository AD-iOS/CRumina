    ...
    if let Ok(Some(value)) = result {
        match value {
            Value::Float(f) => {
                assert!(
                    (f - 0.1).abs() < 1e-10,
                    "decimal(1/10) should be 0.1, got {}",
                    f
                );
            }
    ...
    if let Ok(Some(value)) = result {
        match value {
            Value::Float(f) => {
                assert!(
                    (f - 0.25).abs() < 1e-10,
                    "decimal(1/4) should be 0.25, got {}",
                    f
                );
            }
            _ => panic!("Expected Float, got {:?}", value),
...
fn test_decimal_function_with_third() {
    ...
    if let Ok(Some(value)) = result {
        match value {
            Value::Float(f) => {
                assert!(
                    (f - 0.333333333333).abs() < 1e-10,
                    "decimal(1/3) should be ~0.333, got {}",
                    f
                );
            }
            _ => panic!("Expected Float, got {:?}", value),
...
fn test_rational_arithmetic_stays_exact() {
    ...
    if let Ok(Some(value)) = result {
        ...
        assert_eq!(
            display, "3/10",
            "0.1 + 0.2 should display as 3/10, got {}",
            display
        );
    }
}
    ...
    if let Ok(Some(value)) = result {
        match value {
            Value::String(s) => {
                assert_eq!(
                    s, "rational,float",
                    "Types should be rational,float, got {}",
                    s
                );
            }
            _ => panic!("Expected String, got {:?}", value),
...
fn test_complex_rational_expression() {
    ...
    if let Ok(Some(value)) = result {
        ...
        assert_eq!(
            display, "3/10",
            "(0.1 + 0.2) * (0.5 + 0.5) should be 3/10, got {}",
            display
        );
    }
}
